...
export async function prepareApiRequest(): Promise<{
  ...
  if (accessToken === undefined) {
    throw new Error(
      'Claude Code web sessions require Claude.ai authentication. API key authentication is not sufficient.',
    )
  }
...
export async function fetchSession(
  ...
  if (response.status !== 200) {
    ...
    if (response.status === 401) {
      throw new Error('Session expired. Re-authentication is required.')
    }
